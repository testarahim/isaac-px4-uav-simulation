...
if [ -f "$ROOT_DIR/configs/run_mavproxy.sh" ]; then
    ...
    check_contains "$ROOT_DIR/configs/run_mavproxy.sh" "--out=udpout:127.0.0.1:14555" "Gimbal control bridge output"
    check_contains "$ROOT_DIR/configs/run_mavproxy.sh" "--out=udpout:127.0.0.1:14556" "QGC camera component helper output"
fi
